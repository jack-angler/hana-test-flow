...
SELECT
  tr.name,
  COUNT(tc.id) AS current_rows
FROM test_runs tr
INNER JOIN test_scenarios ts ON ts.test_run_id = tr.id
INNER JOIN test_cases tc ON tc.test_scenario_id = ts.id
WHERE tr.name = '리스(운용, 금융, 중고차, 시승차)'
  AND tc.is_current = 1
  AND tc.is_deleted = 0
GROUP BY tr.name;
